...
export const TranscriptViewerModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-stone-900/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[85vh] rounded-2xl border border-stone-700 bg-[#0f111a] shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        {/* Header */}
          ...
          <div className="flex items-center gap-2">
            {entries.length > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-stone-300 bg-stone-800 hover:bg-stone-700 border border-stone-700 rounded-lg transition-colors"
              >
        ...
        {/* Participants info */}
        <div className="flex items-center gap-4 px-5 py-3 border-b border-stone-800/60 text-[11px] font-medium text-stone-500 shrink-0">
          ...
          <span className="ml-auto text-stone-600">{entries.length} entries</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3 min-h-0">
          ...
          {/* Video Player */}
          {recordingUrl && (
            <div className="mb-6 rounded-xl border border-stone-800 bg-[#151822] overflow-hidden">
              <video 
                src={recordingUrl} 
                controls 
              ...
              />
              <div className="p-3 border-t border-stone-800/60 flex items-center justify-between text-[11px] text-stone-500 font-medium">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-stone-500 font-medium">Loading transcript...</p>
            </div>
          ) : entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-16 h-16 rounded-2xl bg-stone-800 border border-stone-700 flex items-center justify-center mb-4">
                <Mic size={28} className="text-stone-600" />
              </div>
              <p className="text-sm font-bold text-stone-400">No transcript available</p>
            ...
            entries.map((entry, i) => (
              ...
              >
                ...
                <div className={`flex-1 max-w-[80%] ${entry.speaker === 'interviewer' ? 'flex flex-col items-end' : ''}`}>
                  ...
                  <p className="text-[9px] text-stone-700 mt-1 px-1">{entry.timestamp}</p>
                </div>
              </div>
            ))
          )}
