...
import { ScreeningAgentConsole } from '../components/ScreeningAgentConsole';
...
const getStageStyle = (status: string) => {
  ...
  return 'badge-premium bg-stone-50 text-stone-500 border-stone-200';
};
...
const getAiScoreStyle = (score: number) => {
  ...
  if (score >= 55) return { badge: 'badge-premium badge-violet', bar: 'bg-emerald-500' };
  return { badge: 'badge-premium bg-stone-50 text-stone-500 border-stone-200', bar: 'bg-stone-400' };
};
...
const GRADIENTS = [
  'from-blue-600 to-cyan-600', 'from-emerald-600 to-emerald-600',
  'from-amber-600 to-orange-600', 'from-emerald-600 to-teal-600',
  'from-rose-600 to-pink-600', 'from-sky-600 to-emerald-600',
];
  ...
  <div className="panel p-5 animate-pulse">
    <div className="flex items-center gap-4">
      ...
      <div className="flex-1 space-y-2">
        ...
        <div className="h-2.5 bg-stone-100 rounded w-56" />
      </div>
...
export const CompanyApplicationsPage = () => {
  ...
  // ── Data (React Query: cached, deduped, background-refetched) ──
  const applicationsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return unwrapArray(data, ['applications']);
    },
  });
  ...
  useEffect(() => {
    if (applicationsQuery.isError) toast.error('Failed to load applications');
  ...
  const jobsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return unwrapArray(data, ['jobs']);
    },
    staleTime: 5 * 60 * 1000, // job list rarely changes within a session
  });
  ...
  // Feedback for the currently expanded row — fetched once, then cached.
  const feedbackQuery = useQuery({
    queryKey: ['application-feedback', expandedApp],
    queryFn: async () => {
      const { data } = await api.get(`/applications/${expandedApp}/feedback`);
      return Array.isArray(data?.data) ? data.data : [];
    },
    enabled: !!expandedApp,
  });
  // Shims so the render below can stay untouched.
  const feedbackData: Record<string, any[]> =
    expandedApp && feedbackQuery.data ? { [expandedApp]: feedbackQuery.data } : {};
  const loadingFeedback =
    expandedApp && feedbackQuery.isLoading ? expandedApp : null;

  const stageMutation = useMutation({
    mutationFn: ({ id, status, notes }: { id: string; status: string; notes?: string }) =>
      api.patch(`/applications/${id}/stage`, { stage: status, notes }),
  ...
  const handleStageChange = (id: string, status: string, notes = '') =>
    stageMutation.mutateAsync({ id, status, notes });
  ...
  const toggleFeedback = (appId: string) => {
    // The feedback query is keyed on expandedApp and cached, so re-expanding a
    // row it has already loaded is instant with no extra request.
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="topbar-frost px-6 py-4 sticky top-0 z-40">
        <div>
          <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider flex items-center gap-1.5 mb-0.5">
            Workspace &rsaquo; <span className="text-stone-600">Applications</span>
          </p>
      ...
      <main className="p-6 max-w-[1400px] w-full mx-auto space-y-6 flex-1 animate-fade-in-up">
        ...
        <div className="panel p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
            ...
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-400 pointer-events-none" size={13} />
              <input
                type="text"
                placeholder="Search candidates or job posts..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="input-premium pl-9 text-xs py-2 bg-stone-50/50"
              />
            </div>

            {/* Job filter */}
            <div className="relative">
              <SlidersHorizontal className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 pointer-events-none" size={12} />
              <select
                value={selectedJob}
                onChange={(e) => setSelectedJob(e.target.value)}
                className="input-premium pl-8 pr-8 py-2 text-xs bg-stone-50/50 font-bold text-stone-600 appearance-none min-w-[180px]"
              >
              ...
              <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 pointer-events-none" size={12} />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {selectedIds.size === 1 && (
              <span className="text-[10px] text-stone-500 font-bold px-3 py-2 bg-stone-50 border border-stone-200/60 rounded-xl">
        ...
        {loading ? (
          ...
          <div className="py-24 text-center panel">
            ...
            <h3 className="text-sm font-bold text-stone-700 mt-1" style={{ fontFamily: 'Plus Jakarta Sans' }}>No Applications Found</h3>
            <p className="text-xs text-stone-400 font-medium mt-1.5 max-w-xs mx-auto">Try adjusting your filters or search terms.</p>
          </div>
        ) : (
          <div className="panel divide-y divide-stone-100 overflow-hidden list-slide-in">
            {filtered.map((app, i) => {
              ...
              return (
                <div
                  key={app.id || i}
                  className="overflow-hidden transition-colors duration-200 hover:bg-stone-50/20"
                >
                  {/* ── Main row ── */}
                  <div className="p-4 sm:p-5 flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-3.5 flex-1 min-w-0">
                      <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-white font-extrabold text-[11px] shrink-0 shadow-sm border border-stone-700/5`}>
                        {initials}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-sm font-bold text-stone-900 leading-none">{name}</p>
                          <span className={getStageStyle(app.status)}>
                            {getStageLabel(app)}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 text-[10.5px] text-stone-400 font-semibold flex-wrap mt-1">
                          <span className="text-stone-700 truncate max-w-[180px]">{app.job_title || '—'}</span>
                    ...
                    <div className="flex items-center gap-4 shrink-0 justify-between md:justify-end">
                      ...
                      <span className="text-[10px] text-stone-400 font-bold uppercase tracking-wider whitespace-nowrap">{date}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-stone-50 text-stone-650 border border-stone-200 rounded-xl text-[10.5px] font-bold transition-spring shadow-sm h-8.5 hover:border-stone-300 cursor-pointer"
                      >
                        <Eye size={12} className="text-stone-450" />
                        Feedback
                        {isExpanded
                          ? <ChevronUp size={11} className="text-stone-450" />
                          : <ChevronDown size={11} className="text-stone-450" />}
                      </button>
                      ...
                      {(status.startsWith('interview_') || status === 'interview') && (() => {
                        if (latestStatus === 'scheduled' || latestStatus === 'reschedule_requested') {
                          return (
                            <>
                              <span className="h-8.5 inline-flex items-center px-3 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-xl text-[10.5px] font-bold">
                  ...
                  {/* ── Feedback drawer ── */}
                  {isExpanded && (
                    <div className="border-t border-stone-100 bg-stone-50/40 p-5 space-y-4 animate-fade-in-up">
                      ...
                      {loadingFeedback === app.id ? (
                        <div className="flex items-center gap-2 py-4">
                          ...
                          <span className="text-xs text-stone-450 font-bold">Retrieving scorecards...</span>
                        </div>
                      ) : appFeedback.length === 0 ? (
                        <div className="py-8 text-center bg-white border border-stone-150 rounded-2xl max-w-md mx-auto shadow-sm">
                          <MessageSquare size={20} className="mx-auto text-stone-300 mb-2" />
                          <p className="text-xs font-bold text-stone-500">No scorecards submitted</p>
                          <p className="text-[10px] text-stone-400 mt-0.5">Feedback details will load after the interviews are completed.</p>
                        </div>
                      ) : (
                                ...
                                <div className="inner-core space-y-3">
                                  ...
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stone-100">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        <div className="flex items-center gap-2">
                                          <p className="text-xs font-extrabold text-stone-900 capitalize">
                                        ...
                                        <div className="flex items-center gap-1.5 text-[10px] text-stone-400 font-semibold mt-0.5 flex-wrap">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      {f.recommendation && (
                                        <span className={REC_STYLE[f.recommendation] || 'badge-premium bg-stone-100 text-stone-650'}>
                                    ...
                                    <div className="flex items-center gap-4 p-3.5 rounded-xl bg-emerald-50/70 border border-emerald-100">
                                      ...
                                      <div className="flex-1">
                                        <div className="w-full bg-emerald-100/50 rounded-full h-1.5 mb-1.5">
                                          <div
                                            className={`h-1.5 rounded-full transition-spring ${aptitudePct != null && aptitudePct >= 70 ? 'bg-emerald-500' : 'bg-emerald-500'}`}
                                            style={{ width: `${aptitudePct ?? 0}%` }}
                                          />
                                        </div>
                                        <p className={`text-[10px] font-bold ${aptitudePct != null && aptitudePct >= 70 ? 'text-emerald-600' : 'text-emerald-650'}`}>
                                  ...
                                  {hasFeedback && (
                                    <>
                                      <div className="flex items-center gap-1">
                                        ...
                                        <span className="ml-1.5 text-[10.5px] font-bold text-stone-500">
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div className="p-3.5 rounded-xl bg-emerald-50/30 border border-emerald-100/60">
                                          ...
                                          <p className="text-xs font-medium text-stone-700 leading-relaxed">{f.strengths || 'Not provided'}</p>
                                        </div>
                                        <div className="p-3.5 rounded-xl bg-rose-50/20 border border-rose-100/50">
                                          ...
                                          <p className="text-xs font-medium text-stone-700 leading-relaxed">{f.weaknesses || 'Not provided'}</p>
                                        </div>
                                      </div>

                                      {f.additional_comments && (
                                        <div className="p-3.5 rounded-xl bg-stone-50 border border-stone-100">
                                          <p className="text-[9px] font-black uppercase tracking-wider text-stone-500 mb-1">Notes</p>
                                          <p className="text-xs font-medium text-stone-655 leading-relaxed italic">&ldquo;{f.additional_comments}&rdquo;</p>
                                        </div>
                                      )}
                                    </>
                                  )}
      ...
      {selectedJob !== 'all' && (
        <ScreeningAgentConsole
          jobId={selectedJob}
          open={screenOpen}
          onClose={() => { setScreenOpen(false); refetchApplications(); }}
        />
      )}
    </div>
  );
