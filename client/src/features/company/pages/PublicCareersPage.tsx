        ...
        <div className="card-premium p-4 mb-8 flex flex-col sm:flex-row gap-4 shadow-xl shadow-slate-200/50">
           <div className="flex-1 relative group">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:ring-4 focus:ring-amber-500/5 focus:border-amber-400 outline-none transition-all font-bold text-slate-700 text-sm"
              />
           </div>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredJobs.map((job) => (
              <Link 
                key={job.id}
                to={`/jobs/${job.id}`}
                className="card-premium p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 group hover:border-amber-200 transition-all duration-300"
              >
                <div className="space-y-3 flex-1">
                  <div className="flex items-center gap-3">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight group-hover:text-amber-600 transition-colors">{job.title}</h3>
                    <span className="px-2 py-1 bg-amber-50 text-amber-600 text-[9px] font-black uppercase tracking-wider rounded border border-amber-100">
                ...
                <div className="shrink-0 flex items-center gap-4 border-t sm:border-t-0 sm:border-l border-slate-100 pt-4 sm:pt-0 sm:pl-8">
                   ...
                   <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-white group-hover:bg-amber-500 group-hover:translate-x-1 transition-all">
                      <ArrowRight size={20} />
                   </div>
                </div>
              </Link>
