...
const ASSESSMENT_STATUS_CONFIG = {
  not_started: { label: 'Not Started', color: 'bg-stone-100 text-stone-600 border-stone-200' },
      ...
      <div className="topbar-frost px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-40">
        <div>
          <div className="text-xs text-stone-400 font-medium flex items-center gap-1.5 mb-1">
            <span>Candidate</span>
            <span>&rsaquo;</span>
            <span className="text-stone-600 font-semibold">Interviews</span>
          </div>
          <div className="flex items-baseline gap-2">
            <h1 className="text-xl font-bold text-stone-900 tracking-tight" style={{ fontFamily: 'Plus Jakarta Sans, Inter, sans-serif' }}>
              My Interviews
            </h1>
            <span className="text-xs text-stone-400 font-medium">{interviews.length} total</span>
          </div>
        </div>
      </div>
      ...
      <main className="p-5 max-w-[1400px] w-full mx-auto flex-1 flex flex-col">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
          ...
          <div className="space-y-4">
            <div className="flex bg-stone-100 border border-stone-200/40 p-0.5 rounded-lg w-fit">
              <button
            ...
            <div className="space-y-3">
              {loading ? (
                [1, 2, 3].map(i => <div key={i} className="h-28 rounded-xl bg-white border border-stone-200 animate-pulse" />)
              ) : filteredInterviews.length === 0 ? (
                <div className="text-center py-20 surface-raised !rounded-xl max-w-xl mx-auto w-full">
                  <Calendar size={36} className="mx-auto text-stone-250 mb-3" />
                  <h3 className="text-sm font-bold text-stone-800" style={{ fontFamily: 'Plus Jakarta Sans, Inter, sans-serif' }}>No interviews found</h3>
                  <p className="text-xs text-stone-400 mt-1">You don't have any {activeTab} interviews at the moment.</p>
                </div>
              ) : (
                filteredInterviews.map((interview) => {
                  ...
                  return (
                    <div key={interview.id} className="bg-white rounded-xl border border-stone-200/80 p-5 flex flex-col md:flex-row gap-5 items-start md:items-center justify-between shadow-sm hover:shadow-md hover:border-stone-350 transition-all">
                      ...
                      <div className="flex items-center gap-4 min-w-[200px]">
                        ...
                        <div>
                          <h4 className="font-bold text-sm text-stone-900 leading-tight">{interview.company_name}</h4>
                          <p className="text-[10px] font-extrabold uppercase tracking-wider mt-1 flex items-center gap-1">
                            <span className={automated ? 'text-emerald-600' : 'text-emerald-600'}>
                      ...
                      {/* Middle: Details */}
                      <div className="flex-1 flex flex-wrap gap-x-6 gap-y-3 text-xs font-semibold text-stone-650 w-full md:w-auto md:px-4">
                        {automated ? (
                          <>
                            <div className="space-y-0.5 min-w-[120px]">
                              <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                <Cpu size={11} className="text-stone-400" /> Type
                              </p>
                              <p className="text-stone-800 font-bold capitalize">
                                {interview.round_type === 'technical' ? 'Coding Assessment' : 'Aptitude Test'}
                              </p>
                            </div>
                            <div className="space-y-0.5 min-w-[100px]">
                              <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                <Clock size={11} className="text-stone-400" /> Duration
                              </p>
                              <p className="text-stone-800 font-bold">{interview.duration} min</p>
                            </div>
                          ...
                          <>
                            {date && (
                              <div className="space-y-0.5 min-w-[100px]">
                                <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                  <Calendar size={11} className="text-stone-400" /> Date
                                </p>
                                <p className="text-stone-800 font-bold">{date}</p>
                              </div>
                            )}
                            {time && (
                              <div className="space-y-0.5 min-w-[70px]">
                                <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                  <Clock size={11} className="text-stone-400" /> Time
                                </p>
                                <p className="text-stone-800 font-bold">{time}</p>
                              </div>
                            )}
                            <div className="space-y-0.5 min-w-[90px]">
                              <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                <Video size={11} className="text-stone-400" /> Format
                              </p>
                              <p className="text-stone-800 font-bold capitalize">{interview.round_type || 'Interview'}</p>
                            </div>
                            <div className="space-y-0.5 min-w-[120px]">
                              <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                <User size={11} className="text-stone-400" /> Host
                              </p>
                              <p className="text-stone-800 font-bold truncate max-w-[130px]" title={interview.interviewer_name}>
                      ...
                      <div className="flex items-center gap-2 shrink-0 w-full md:w-auto justify-end">
                        ...
                        {!automated && isScheduled && (
                          <>
                            <button
                              onClick={() => navigate(`/interview/${interview.id}`)}
                              className="h-8 px-3.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[10.5px] font-bold rounded-lg transition-all flex items-center gap-1 shadow-sm"
                            >
                              <Video size={12} />
                              Join
                            </button>
                            <button
                              ...
                              className="h-8 px-3 border border-stone-200 text-stone-650 hover:bg-stone-50 text-[10.5px] font-bold rounded-lg transition-all flex items-center gap-1 bg-white shadow-sm"
                            >
                        ...
                        {!automated && !isScheduled && !isPendingReschedule && (
                          <span className="inline-flex items-center px-3 py-1 rounded-full bg-stone-500/10 text-stone-500 border border-stone-500/20 text-[9px] font-semibold uppercase tracking-wider">
          ...
          <div className="space-y-6">
            <div className="bg-white border border-stone-200/80 rounded-xl p-5 shadow-sm space-y-4">
              <div className="flex items-center gap-1.5 border-b border-stone-100 pb-3">
                <Sparkles size={13} className="text-emerald-500 animate-float" />
                <h3 className="text-[10px] font-semibold uppercase text-stone-400 tracking-wider">
                  AI prep assistant
                </h3>
              </div>
              ...
              {activePrepInterview ? (
                <div className="space-y-4">
                  <div className="bg-stone-50 rounded-lg p-3 border border-stone-100">
                    <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">Target Round</p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {getPrepQuestions(activePrepInterview.round_type).map((q, idx) => (
                        <div key={idx} className="p-2.5 rounded-lg bg-stone-50 border border-stone-100 text-[10.5px] text-stone-700 font-medium leading-normal hover:bg-stone-100/50 transition-colors">
                          {q}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="pt-2">
                    <div className="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-2">Prep Checklist</div>
                    <div className="space-y-1.5 text-[10.5px] text-stone-600 font-medium">
                      {AUTOMATED_ROUNDS.includes(activePrepInterview.round_type) ? (
                        <>
                          ...
                          <div className="flex items-center gap-2"><Lock size={12} className="text-stone-300" /> Do not switch tabs during the test</div>
                        </>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center gap-2"><CheckCircle2 size={12} className="text-stone-300" /> Research {activePrepInterview.company_name}'s values</div>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div className="text-center py-6 text-stone-400 text-xs">
                  <Compass size={24} className="mx-auto text-stone-200 mb-2" />
            ...
            <div className="bg-white border border-stone-200/80 rounded-xl p-5 shadow-sm space-y-3">
              <h4 className="text-[10px] font-semibold uppercase text-stone-400 tracking-wider">Hiring Tips</h4>
              <ul className="space-y-2.5 text-[10.5px] text-stone-650 font-medium">
